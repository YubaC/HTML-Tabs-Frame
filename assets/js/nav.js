...
function slideXToActiveTab($activeTab) {
  ...
  var $navItemScrollLeft = $navItemOffsetLeft - $navOffsetLeft + $navScrollLeft;
...
function changeTabContent(src) {
  ...
  if ($contentBox.length !== 0) {
    ...
    $contentBox.removeClass(contentBoxClass).addClass(activeContentBoxClass);
    // *这个方法暂不可用
    // // 如果是iframe元素就解冻它
...
function searchTabs() {
  ...
  $navViewAllBoxTabList.children().each(function () {
    ...
    if (
      $(this).text().toLowerCase().indexOf(searchContent.toLowerCase()) != -1
    ) {
